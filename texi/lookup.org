...
You can create, delete and edit modules by /Module List/ Buffer. (~M-x lookup-list-modules~)
...
You can create a function to search with a specific module as below.
The below function does ~lookup-pattern~ with "english" module.

#+BEGIN_SRC emacs-lisp
  (defun lookup-pattern-module-english ()
    (interactive)
    (let ((module (lookup-get-module "english")))
      (lookup-pattern (lookup-input-pattern module) module)))
#+END_SRC
